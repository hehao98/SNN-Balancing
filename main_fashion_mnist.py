...
def get_trained_model(train_data, test_data, model_type):
    if os.path.exists(f"model_{model_type}_fashion.pt"):
        model = torch.load(f"model_{model_type}_fashion.pt")
        with open(f"model_{model_type}_perf_fashion.json", "r") as f:
            perf = json.load(f)
        return model, perf
    if model_type == "mlp":
        model = get_model_mlp()
    ...
    torch.save(model, f"model_{model_type}_fashion.pt")
    with open(f"model_{model_type}_perf_fashion.json", "w") as f:
...
if __name__ == "__main__":
    ...
    mnist_train, mnist_test = get_fashion_mnist()
    ...
    with open("result-fashion.json", "w") as f:
        json.dump({
    ...
    ax.legend()
    fig.savefig("result-fashion.png", bbox_inches="tight", dpi=300)
